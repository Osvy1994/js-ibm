...
const findBySpecial = tech => {
  const foundEmployeeByTech = employees.find(
    employee => employee.specialization === tech,
  )
  if (foundEmployeeByTech) {
    document.getElementById('employeesDetails').innerHTML =
      `<p>${foundEmployeeByTech.id}: ${foundEmployeeByTech.name}: ${foundEmployeeByTech.name} - ${foundEmployeeByTech.department} - $${foundEmployeeByTech.salary}</p>`
  }
}
